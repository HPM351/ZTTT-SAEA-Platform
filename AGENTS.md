...
## 11. 编码准则（Karpathy Guidelines）

源自 Andrej Karpathy 对 LLM 编码常见错误的总结。以下准则适用于本项目所有代码修改，每次改动必须遵守。
...
- 动手前先陈述假设。如果不确定，主动追问。
- 如果存在多种解读，全部列出来，不要自己默默选一个。
- 如果有更简单的方案，说出来。必要时合理反驳需求。
- 如果某处不理解，停下来。明确说出哪里不清楚，然后问。
...
强成功标准 = 能自主闭环。弱标准 = 需要反复确认。

权衡提示：这些准则偏向谨慎而非速度。对于无关紧要的小任务，自行判断。
